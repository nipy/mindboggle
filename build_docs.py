...
_info_fname = pjoin(os.path.dirname(__file__), 'mindboggle', 'info.py')
INFO_VARS = {}
...
################################################################################
# Distutils Command class for installing mindboggle to a temporary location.
class TempInstall(Command):
    temp_install_dir = os.path.join('build', 'install')

    def run(self):
        """ build and install mindboggle in a temporary location. """
        ...
        # Horrible trick to reload mindboggle with our temporary instal
        for key in sys.modules.keys():
            if key.startswith('mindboggle'):
