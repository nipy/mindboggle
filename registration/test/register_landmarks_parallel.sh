...
## echo " bash $0 -d 3 -m 30x50x20 -t GR  -s CC -c 1 -o MY -z InitialTemplate.nii.gz  *RF*T1x.nii.gz "
 echo " bash register_landmarks_parallel.sh -d 3 -i 1 -m 1x0x0 -t GR -s CC -g 0.25 -o MY -a /hd2/data/Archive/landmark_registration_evaluation_2011_data_output/data/S05_labelshells -b /hd2/data/Archive/landmark_registration_evaluation_2011_data_output/data/S20_labelshells -e ./output -c 2 -j 4 *.nii.gz"
...
Optional arguments:
     ...
     -z:  Use this volume as the target of all inputs. When not used, the script
...
--------------------------------------------------------------------------------------
 ...
 Atlas directory:           $ATLAS_DIR
 Input Brain directory:     $INPUT_DIR
...
## @rno
    ${ANTSPATH}WarpImageMultiTransform ${dim} atlas_{outputname}warp.nii.gz atlas_{outputname}warp.nii.gz -i ${templatename}Affine.txt ${templatename}warp.nii.gz ${templatename}warp.nii.gz ${templatename}warp.nii.gz ${templatename}warp.nii.gz -R ${template} 
...
while getopts "a:b:c:d:e:g:i:j:h:m:n:o:s:r:t:z:" OPT 
  ...
  case $OPT in
	  ...
	  exit 0 
	  ;;
      a) # atlas directory (@rno)
	  ATLAS_DIR=$OPTARG 
	  ;;
      b) # input brain directory (@rno)
	  INPUT_DIR=$OPTARG 
	  ;;
      c) #use SGE cluster 
	  DOQSUB=$OPTARG 
...
# fi
  ...
  for IMG in $IMAGESETVARIABLE 
    ...
    # 4 Test if outputfilename has only a single extension and remove that
    if [ ${#OUTFN} -eq ${#POO} ]
	then
...
##  exe="${ANTSSCRIPTNAME} -d ${DIM} -r ${dir}/${TEMPLATE} -i ${dir}/${IMG} -o ${dir}/${OUTFN} -m ${MAXITERATIONS} -n ${N3CORRECT} -s ${METRICTYPE} -t ${TRANSFORMATIONTYPE} "
    exe="${ANTSSCRIPTNAME} -d ${DIM} -r ${atlas_dir}/${IMG} -i ${input_dir}/${IMG} -o ${output_dir}/${OUTFN} -m ${MAXITERATIONS} -s ${METRICTYPE} -t ${TRANSFORMATIONTYPE} "
