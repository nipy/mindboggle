...
NOTE: This is the second test to see what range of parameters
...
outpath = "output/"
results = 'results/test2_registration_average_overlaps.csv'
ext = ".nii.gz"
...
#
dim = 2
gradient_step_sizes = [0.4, 0.5, 0.6, 0.75]
iterations = "100x100"
...
# Landmark parameters
#
landmark_weights = [0.1, 0.25, 0.50, 1.0]
landmark_measure = "PSE"
if landmark_measure == "PSE":
    percent = 0.99  # real number: 0.99 = 100%
    boundary = 0  # 0: not only boundaries
    sigmas = [20,100] # big numbers are nearly uniform distributions
    neighbors = [10,20,100]
    matching_iters = [100000] # partial matching iterations
                ...
                if os.path.exists(source) and os.path.exists(target) and \
                   os.path.exists(source_landmarks) and os.path.exists(target_landmarks):
                    ...
                    if compare_labels:
                        ...
                        average_jaccard = 0
                        for label in labels:
                            args4 = " ".join(['c3d', output+'_labels'+ext, target_labels, '-overlap', str(label), '>output/temp2_overlap.txt'])
                            if verbose: print(args4)
                            p = call(args4, shell="True")
                            f = open('output/temp2_overlap.txt','r')
                            temp = f.read()
                            if average_overlaps:
