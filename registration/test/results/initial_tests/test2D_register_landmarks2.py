...
NOTE: This test follows two initial tests to see what range of parameters
...
outpath = "output/"
results = 'results/test2D_average_overlaps2.csv'
ext = ".nii.gz"
...
if regularizer == "DMFFD":
    regularizer_settings = ['12x6']  # DMFFD iterations
    bspline_order = 3
...
#
intensity_weights = [0.1, 0.2]
intensity_measure = "CC"
if intensity_measure == "CC":
    intensity_settings = [4, 5, 6]  # radius
...
# Landmark parameters
#
landmark_weights = [0.3, 0.5]
landmark_measure = "PSE"
if landmark_measure == "PSE":
    percent = 0.99  # real number: 0.99 = 100%
    boundary = 0  # 0: not only boundaries
    sigmas = [5] # big numbers are nearly uniform distributions
    neighbors = [50]
    matching_iters = [0, 1000] # partial matching iterations
          ...
          for sigma in sigmas:
            for neighbor in neighbors:
              for matching_iter in matching_iters:
                ...
                args0 = ['test'+str(count),gradient_step_size,intensity_weight,intensity_setting,landmark_weight,sigma,neighbor,matching_iter]
