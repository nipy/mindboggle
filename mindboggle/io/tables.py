...
def write_columns(columns, column_names, delimiter=',', quote=True,
    ...
    Examples
    --------
    >>> from mindboggle.io.tables import write_columns
...
def write_shape_stats(labels_or_file=[], sulci=[], fundi=[],
    ...
    >>> from mindboggle.io.tables import write_shape_stats
...
def write_vertex_measures(output_table, labels_or_file, sulci=[], fundi=[],
    ...
    >>> import os
    >>> from mindboggle.io.vtks import read_scalars
    >>> from mindboggle.io.tables import write_vertex_measures
    >>> #
...
def write_face_vertex_averages(input_file, output_table='',
    ...
    --------
    >>> import os
    >>> from mindboggle.io.tables import write_face_vertex_averages
    >>> path = os.environ['MINDBOGGLE_DATA']
...
def write_average_face_values_per_label(input_indices_vtk,
    ...
    --------
    >>> import os
    >>> from mindboggle.io.tables import write_average_face_values_per_label
    >>> path = os.environ['MINDBOGGLE_DATA']
...
def alternate_columns_from_tables(table_files, write_table=True,
    ...
    Examples
    --------
    >>> from mindboggle.io.tables import alternate_columns_from_tables
...
def select_column_from_tables(tables, column_name, label_name='',
    ...
    Examples
    --------
    >>> from mindboggle.io.tables import select_column_from_tables
...
def select_column_from_mindboggle_tables(subjects, hemi, tables_dir,
    ...
    --------
    >>> import os
    >>> from mindboggle.io.tables import select_column_from_mindboggle_tables
...
def concatenate_mindboggle_tables(subjects, hemi, tables_dir,
    ...
    --------
    >>> import os
    >>> from mindboggle.io.tables import concatenate_mindboggle_tables
    >>> subjects = ['Twins-2-1', 'Twins-2-2']
    >>> hemi = 'left'
    >>> tables_dir = os.path.join(os.environ['HOME'], 'mindboggled')
    >>> table_name = "label_shapes.csv"
