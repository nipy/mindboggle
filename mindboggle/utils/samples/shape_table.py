#!/usr/bin/python
"""
Construct tables of shape measures for labeled surface patches
and extracted features.

NOTE:  The load_multi_segs takes really long to run,
...
def load_multi_segs(Path, Suffix, Quit=1000):
    ...
    LUT: all SCALARS loaded from a segmented fundi file
        ...
        8. Segment indices in the hemisphere 7  -- no need to keep
...
def common_pairs(LabelPair_All):
    """
    Load segmented fundi  and leave only those that appear in all hemispheres
...
def return_other_element_in_tuple(one_element, Tuple):
    """
    ...
    ======
    one_element: integer
    Tuple: list of length two containing one_element

    """

    if one_element == Tuple[0]:
        return Tuple[1]
    elif one_element == Tuple[1]:
        return Tuple[0]
    else:
        return -1
    ...
    while Extended:
        ...
        for Vrtx in Spanned:
            for Line in Fundus_Line:
                Another = return_other_element_in_tuple(Vrtx, Line)
                if Another > -1:
