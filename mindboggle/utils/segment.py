...
#-----------------------------------------------------------------------------
# Segment voxels by their nearest labels
#-----------------------------------------------------------------------------
def segment_volume(source, target, output_file='', ignore_label=0):
    """
    Segment target voxels by their nearest source voxels' labels.
    ...
    target : string
        unlabeled nibabel-readable (e.g., nifti) file
    output_file : string
        labeled nibabel-readable (e.g., nifti) file
    ignore_labels : integer or float
        source label to ignore (e.g., background value)
    ...
    >>> import os
    >>> from mindboggle.utils.segment import segment_volume
    >>> from mindboggle.utils.plots import plot_volumes
    >>> from mindboggle.utils.io_vtk import transform_to_volume
    >>> data_path = os.environ['MINDBOGGLE_DATA']
    >>> target = os.path.join(data_path, 'arno', 'mri', 'mask.nii.gz')
    >>> source_vtk = os.path.join(data_path, 'arno', 'features', 'sulci.vtk')
    >>> source = transform_to_volume(source_vtk, target)
    >>> target = os.path.join(data_path, 'arno', 'mri', 'mask.nii.gz')
    >>> output_file = ''
    >>> ignore_label = 0
    >>> output_file = segment_volume(source, target, output_file, ignore_label)
    >>> # View
    ...
    # Load un/labeled image volumes:
    vol_source = nb.load(source)
    vol_target = nb.load(target)
    xfm = vol_target.get_affine()
    data_source = vol_source.get_data()
    data_target = vol_target.get_data()

    # Initialize output:
    new_data = data_target.copy()

    # Find source and target voxel coordinates:
    Isource = np.argwhere(data_source != ignore_label)
    xyz_source = np.argwhere(data_source != ignore_label)
    xyz_target = np.argwhere(data_target != ignore_label)
    len_source = len(xyz_source)
    len_target = len(xyz_target)
    unique_labels = [x for x in np.unique(data_source) if x != -1]

    # Construct each seed list from the voxels for a single label:
    seed_lists = []
    for label in unique_labels:
        seed_lists.append([x for i,x in enumerate(indices_borders) if np.sort(label_pairs[i]).tolist() in label_pair_list])

    # Select the first voxel:
    seed_lists = [[xyz_target[0]]]
    print('    Segment {0} voxels from the first voxel as initial seed'.
          format(len_target))

    #-------------------------------------------------------------------------
    # Initialize variables, including the list of voxel indices for each region,
    # voxel indices for all regions, and Boolean list indicating which regions
    # are fully grown, number of segments, etc.:
    #-------------------------------------------------------------------------
    segments = -1 * np.ones(len_target)
    region_lists = [[] for x in seed_lists]
    all_regions = []
    fully_grown = [False for x in seed_lists]
    new_segment_index = 0
    ...
    # Segment target mask with nearest source labels:
    for ixyz, xyz in enumerate(xyz_target):
        I = [x[0] for x in IX]
        X = [x[1] for x in IX]
        new_data[I] = X
