...
def write_vtk(output_vtk, points, indices=[], lines=[], faces=[],
    ...
    from mindboggle.io.vtks import write_header, write_points, \
...
def explode_scalars(input_indices_vtk, input_values_vtk='', output_stem='',
    ...
    >>> import os
    >>> from mindboggle.io.vtks import explode_scalars
    >>> from mindboggle.io.plots import plot_surfaces
    >>> path = os.environ['MINDBOGGLE_DATA']
...
def scalars_checker(scalars, scalar_names):
    ...
    --------
    >>> import numpy as np
    >>> from mindboggle.io.vtks import scalars_checker
...
def read_itk_transform_old(transform_file):
    ...
    --------
    >>> import os
    >>> from mindboggle.io.vtks import read_itk_transform
    >>> path = os.environ['MINDBOGGLE_DATA']
...
def apply_affine_transforms(transform_files, inverse_booleans,
    ...
    Examples
    --------
    >>> from mindboggle.io.vtks import apply_affine_transforms
    >>> from mindboggle.io.plots import plot_surfaces
...
def transform_to_volume(vtk_file, volume_file, output_volume=''):
    ...
    --------
    >>> import os
    >>> from mindboggle.io.vtks import transform_to_volume
...
def freesurfer_surface_to_vtk(surface_file, output_vtk):
    ...
    --------
    >>> import os
    >>> from mindboggle.io.vtks import freesurfer_surface_to_vtk
    ...
    from mindboggle.io.vtks import write_header, write_points, write_faces
...
def freesurfer_curvature_to_vtk(surface_file, vtk_file, output_vtk):
    ...
    --------
    >>> import os
    >>> from mindboggle.io.vtks import freesurfer_curvature_to_vtk
...
def freesurfer_annot_to_vtk(annot_file, vtk_file, output_vtk=''):
    ...
    --------
    >>> import os
    >>> from mindboggle.io.vtks import freesurfer_annot_to_vtk
    >>> path = os.environ['MINDBOGGLE_DATA']
