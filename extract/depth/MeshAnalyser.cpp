...
void MeshAnalyser::WriteIntoFile(char* fileName, char* prop)
{
	...
	/* Added by Forrest 2012-01-04  */
	else if(strcmp("Travel_Depth_and_Curv",prop)==0) 
	{
		this->depth->SetName("depth");
		this->mesh->GetPointData()->SetScalars(this->depth);

		this->curv->SetName("curvature");
		this->mesh->GetPointData()->AddArray(this->curv);
	}
	else if(strcmp("Euclidean_Depth_and_Curv",prop)==0) 
	{
		this->euclideanDepth->SetName("depth");
		this->curv->SetName("curvature");

		this->mesh->GetPointData()->SetScalars(this->euclideanDepth);
		this->mesh->GetPointData()->AddArray(this->curv);
	}
	/* end of Added by Forrest 2012-01-04  */
...
{
    cout<<"Computing Travel Depth Map from New Reference surface"<<endl;
    ...
    cout<<"Computing Travel Depth Map from New Reference surface Done!"<<endl;
}
...
void MeshAnalyser::ComputeEuclideanDepthFromClosed(bool norm)
{
    ...
    cout<<"Computing Euclidean Depth Map from new reference surface"<<endl;
    if(this->closedMesh->GetNumberOfPoints()<1)
    {
        ComputeClosedMesh();
    }
    ...
    cout<<"Computing Euclidean Depth Map from new reference surface Done"<<endl;
