...
Mindboggle_ uses the Sphinx_ documentation generating tool.  Sphinx
translates reST_ formatted documents into html and pdf documents.  
All our documents and docstrings are in reST format, which allows us 
to have both human-readable docstrings when viewed in ipython_, and
...
We also build our documentation website using sphinx_.  
All of the documentation in the ``docs`` directory is included 
on the website.  There are a few files that are website only 
and these are placed in the ``www`` directory.  
The easiest way to view the documentation while editing
...
Mindboggle has adopted the numpy_ documentation standards.  The `numpy
...
reST file
<http://svn.scipy.org/svn/numpy/trunk/doc/HOWTO_DOCUMENT.txt>`_ 
that generates the coding style guideline.

Numpy has a `detailed example
<http://svn.scipy.org/svn/numpy/trunk/doc/EXAMPLE_DOCSTRING.txt>`_ 
