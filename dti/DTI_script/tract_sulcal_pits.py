...
#cmd_ants_tmp = '/data/BI/Toolbox/software/ants_maci_svn606/bin/WarpImageMultiTransform 3 %s %s -R %s -i  %s %s --use-NN'  # Order (in_file, out_file, DTI_first_volume, affineFile, inverseWarpFile)

# Dilating pits
#cmd_dilate_tmp ='/data/BI/Toolbox/software/ants_maci_svn606/bin/ImageMath 3 %s MD %s %s' #(output_file, input_file, dilatation_radius)
...
# Sulcal pits tractography
# 1) Move sulcal pits from T1 to DTI space
# 3) Dilate sulcal pits to 3 voxels diameter
# 4) Preparing seeds
spl.move_sulcalPits_from_T1_to_DTI(sulcal_pits_T1,seed_folder,dti_first_vol,Ants_invWarp,Ants_affine,dilatation_radius)

# 5) Performing tractography
# 5.1) Copying bedpostX data in the temporary folder to perform tractography
cmd = 'mkdir -m 777 -p ' + bedpostX_OUT
os.system(cmd)
cmd = 'cp ' + bedpostX_IN + '/*.* ' + bedpostX_OUT
os.system(cmd)

# 5.2) Generating target text file and counting the number of the seeds
target_file_lines = []
exit_flag = False
counter = 1
while not(exit_flag):
	...
	if os.path.isfile(file_name): # file_name exists
		...
		counter = counter +1
		# 3) Dilate sulcal pits to 3 voxels diameter
		#cmd = cmd_dilate_tmp %(file_name,file_name,dilatation_radius) #(input_file, 	output_file, dilatation_radius)
		#os.system(cmd)
...
# 6) Moving the results to the right place and deleting temporary files
...
#cmd = 'mkdir -m 777 -p ' + final_result_folder
#os.system(cmd)
#cmd = 'chmod -R 777 ' + temp_folder+'OUT/*'
#os.system(cmd)
#cmd = 'chmod -R 666 ' + temp_folder+'OUT/*.*'
#os.system(cmd)

#cmd = 'cp -R -f ' + temp_folder + 'OUT/* ' + main_folder
#os.system(cmd)
