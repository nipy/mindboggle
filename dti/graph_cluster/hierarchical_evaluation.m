% STUDY OF HIERARCHICAL METHOD RESULTS
% Code to evaluate the performances of the hierarchical clustering method.
% Questions:
% - method robustness
...
options.dendrogram_Z_threshold = 0.9;
options.saveImages = true;
options.images_basename = ['hier_',num2str(options.n_cluster),'_',options.hier.distance];
...
cluster_table = zeros(nG,nV);
for g = 1:nG
    disp(['Analyzing graph #',num2str(g)])
    graph_adj_matrix = graph_set(:,:,g);
    options.grapn_number = g;
    cluster_results = applyHierarchical(graph_adj_matrix,options);
    cluster_table(g,:) = cluster_results.unsorted.cluster_indices;
    close
end
figure
...
matched_cluster_table = cluster_index_matching(cluster_table);
subplot(122)
